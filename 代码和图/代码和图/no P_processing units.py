...
def f(T, t, T_0):
    ...
    #
    P_cpu_dynamic=P_gpu_dynamic=I_gc=I_gg=0
    ...
    # 计算AI项
    AI = (x[1] + x[2] * T_soc + x[3] * (T_soc ** 2) + x[4] * (T_soc ** 3)) * x[5] * (np.exp(x[6] / (T_battery + x[7])))
    IIFAI = I * I * Fi_N * AI
    UI = U_s * I_s
...
# 解微分方程
track = sp.odeint(f, y0, t, args=(298,))
...
# 图1: SOC变化
plt.subplot(2, 2, 1)
if SOC_0id is not None:
    # 绘制SOC=0之前的实线部分
    plt.plot(t[:SOC_0id+1], track[:SOC_0id+1, 0] * 100, 'b-', linewidth=2)
    # 绘制SOC=0之后的虚线部分
    plt.plot(t[SOC_0id:], track[SOC_0id:, 0] * 100, 'b--', linewidth=2)
    # 标记SOC=0的点
    soc_value = track[SOC_0id, 0] * 100
    plt.plot(t[SOC_0id], soc_value, 'ro', markersize=8, label='SOC=0')
    # 添加文本标注
    plt.annotate(f'SOC={soc_value:.2f}% at t={t[SOC_0id]:.1f}s',
                xy=(t[SOC_0id], soc_value),
                xytext=(t[SOC_0id] + 500, 0.1),
                arrowprops=dict(facecolor='red', shrink=0.05, width=1.5),
                fontsize=10,
                color='red')
    plt.legend()
else:
    plt.plot(t, track[:, 0] * 100, 'b-', linewidth=2)
...
# 图2: CPU/GPU温度变化
plt.subplot(2, 2, 2)
temp_cg = track[:, 1] - 273  # 转换为摄氏度
if SOC_0id is not None:
    # 绘制SOC=0之前的实线部分
    plt.plot(t[:SOC_0id+1], temp_cg[:SOC_0id+1], 'g-', linewidth=2)
    # 绘制SOC=0之后的虚线部分
    plt.plot(t[SOC_0id:], temp_cg[SOC_0id:], 'g--', linewidth=2)
    # 标记SOC=0对应的温度点
    temp_value = temp_cg[SOC_0id]
    plt.plot(t[SOC_0id], temp_value, 'ro', markersize=8)
    # 添加文本标注
    plt.annotate(f'T={temp_value:.1f}°C at SOC=0',
                xy=(t[SOC_0id], temp_value),
                xytext=(t[SOC_0id] + 500, temp_value),
                arrowprops=dict(facecolor='red', shrink=0.05, width=1.5),
                fontsize=10,
                color='red')
else:
    plt.plot(t, temp_cg, 'g-', linewidth=2)
...
# 图3: 电池温度变化
plt.subplot(2, 2, 3)
temp_battery = track[:, 2] - 273  # 转换为摄氏度
if SOC_0id is not None:
    # 绘制SOC=0之前的实线部分
    plt.plot(t[:SOC_0id+1], temp_battery[:SOC_0id+1], 'r-', linewidth=2)
    # 绘制SOC=0之后的虚线部分
    plt.plot(t[SOC_0id:], temp_battery[SOC_0id:], 'r--', linewidth=2)
    # 标记SOC=0对应的温度点
    temp_value = temp_battery[SOC_0id]
    plt.plot(t[SOC_0id], temp_value, 'ro', markersize=8)
    # 添加文本标注
    plt.annotate(f'T={temp_value:.1f}°C at SOC=0',
                xy=(t[SOC_0id], temp_value),
    ...
    plt.plot(t, temp_battery, 'r-', linewidth=2)
...
# 图4: 屏幕温度变化
plt.subplot(2, 2, 4)
temp_screen = track[:, 3] - 273  # 转换为摄氏度
if SOC_0id is not None:
    # 绘制SOC=0之前的实线部分
    plt.plot(t[:SOC_0id+1], temp_screen[:SOC_0id+1], 'm-', linewidth=2)
    # 绘制SOC=0之后的虚线部分
    plt.plot(t[SOC_0id:], temp_screen[SOC_0id:], 'm--', linewidth=2)
    # 标记SOC=0对应的温度点
    temp_value = temp_screen[SOC_0id]
    plt.plot(t[SOC_0id], temp_value, 'ro', markersize=8)
    # 添加文本标注
    plt.annotate(f'T={temp_value:.1f}°C at SOC=0',
                xy=(t[SOC_0id], temp_value),
    ...
    plt.plot(t, temp_screen, 'm-', linewidth=2)
